...
for i in 1:n_iter
    ...
    for j in 1:n_cycles
        ...
        myrandompoints = StatsBase.sample(rng,x_test_random,n_samples)


        # construct generated y-values
        y_train = map(x -> unpackconstruct(x) |> (x -> quad(x,Q,λ)),construct_train_BO)
        # use BO
        ...
        new_predictions = save_thompson_sampling(Model.GP_model,x_test_BO,10,10^-6)
        ...
        #max values
        max_random[i,j] =  maximum(map(x -> unpackconstruct(x) |> (x -> quad(x,Q,λ)),construct_train_random))
        max_model[i,j] = maximum(map(x -> unpackconstruct(x) |> (x -> quad(x,Q,λ)),construct_train_BO ))
